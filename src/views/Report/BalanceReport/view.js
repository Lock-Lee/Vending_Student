...
class View extends React.Component {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.setState(
          ...
          async () => {
            const data = await report_model.sendBalanceReport({
  ...
  _fetchData() {
    this.setState(
      ...
      async () => {
        const data = await report_model.getBalanceReport({
          date_start: time_format.dateToStr(this.state.date_start),
          date_end: time_format.dateToStr(this.state.date_end),
        });
        ...
        for (let index = 0; index < data.data.length; index++) {
          ...
          cost_sum +=
            (parseInt(element.stock_balance) +
              parseInt(element.stock_in) -
              parseInt(element.stock_out)) *
                  ...
                  <tbody>
                    {this.state.data?.map((item, index) => {
                      ...
                      return (
                        <tr
                          style={{
                            color: stock_amount < 0 ? "red" : "#3c4b64",
                          }}
                        >
                          ...
                          <td style={{ textAlign: "right" }}>
                            {number_format.strFix(parseInt(stock_amount))}
                          </td>
                          <td style={{ textAlign: "right" }}>
