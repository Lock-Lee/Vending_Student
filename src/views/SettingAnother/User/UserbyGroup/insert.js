...
class Insert extends React.Component {
  ...
  async _fetchData() {
    const now = new Date();
    const last_code = await usergroup_model.generateUserGroupLastCode({
      code: `UGC${now.getFullYear()}${(now.getMonth() + 1)
        .toString()
        .padStart(2, "0")}`,
      digit: 3,
    });
    ...
    this.setState({
      loading: false,
      user_group_code: last_code.data,
    });
  }
  ...
  _handleSubmit(event) {
    ...
    if (this._checkSubmit()) {
      this.setState(
        ...
        async () => {
          const res = await usergroup_model.insertUserGroup({
          ...
          if (res.require) {
            ...
            this.props.history.push("/settinganother/user/user-group");
          } else {
            this.setState(
              {
                loading: false,
  ...
  _checkSubmit() {
    if (this.state.user_group_code === "") {
      ...
      return false;
    } else if (this.state.user_group_code === "") {
      Swal.fire({
        title: "กรุณาระบุชื่อ !",
  ...
  _inputdata = (e) => {
    // if (this.state.title_modal === "ชื่อกลุ่ม") {
    //   this.setState({
    //     user_group_name: e,
    //     show_modal: false,
    //   });
