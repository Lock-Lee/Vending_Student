...
class Insert extends React.Component {
  ...
  async componentDidMount() {
    ...
    const success = await success_model.getSuccessByCode({
      success_code: code,
    });
    const {
      success_code,
    ...
    this._Selecjob_op(job_code);
    this._Selecjob_op_tools(job_op_code);

    this.setState({
      success_code: success_code,
      prodcut: prodcut.data,
      job_code: job_code,
      job_op_code: job_op_code,
      ...
      success_image: {
        src: `${GLOBAL.BASE_SERVER.URL_IMG}${
          success_image === "" ? "default.png" : success_image
        }`,
        file: null,
        old: success_image,
      },
      ...
      success_partname: success_partname,
    ...
    this.setState(
      ...
      async () => {
        if (this._checkSubmit()) {
          const res = await success_model.updateSuccessBy({
                ...
                <Col>
                  ...
                  <Row>
                    <Col md={6}>
                      {" "}
                      <label>
                        รายละเอียด <font color="#F00"></font>
                      </label>
                      <Input
                        type="textarea"
                        name="text"
