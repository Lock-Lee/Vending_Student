...
export class Assign_product_qty extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  _onSaveProductqty() {
    ...
    if (parseInt(keyword) < 0 || parseInt(keyword) == 0) {
      Swal.fire({ title: "กรุณาตรวจสอบความถูกต้อง ! ", icon: "warning" });
    ...
    else if (keyword <= product_amount && keyword != "") {
      this.props._issueQty(this.state.keyword);
    }
  }
      ...
      <>
        ...
        <ModalFooter style={{ display: "block" }}>
          <Button
            ...
            onClick={() => this._onSaveProductqty()}
          >
          ...
          <Button
            ...
            onClick={() =>
              this.props._displayback({
                event_page_current: 'event_page_assign_product',
              })
            }
          >
...
export default Assign_product_qty;
