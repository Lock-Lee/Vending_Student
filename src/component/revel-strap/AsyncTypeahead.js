...
export default class RevelAsyncTypeahead extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isValid: false,
      defaultSelected: [],
      useCache: this.props.useCache ? this.props.useCache : true,
  ...
  render() {
    ...
    if (this.props.defaultSelected === undefined) {
      ...
      if ((this.props.value === '' && this.props.defaultSelected === '') || this.state.defaultSelected.length) {
        return <AsyncTypeahead {...props} defaultSelected={this.state.defaultSelected} />
      } else {
        return <input type="text" className="form-control" autoComplete="off" placeholder={this.props.placeholder || 'ค้นหา..'} />
      }
    }
  }
}